...
def get_proxy_address():
    res = requests.get('https://free-proxy-list.net/anonymous-proxy.html') # 프록시 사이트 
    html = res.text
...
def get_html_from_proxy(link, params = {}):
    ...
    while proxy_success == False: # 접속이 성공할 때까지 
        ...
        try:
            proxy = random.choice(proxies_list) # 가져온 프록시 리스트 중에 하나를 랜덤 선택
            ...
            headers = {'User-Agent': random.choice(user_agent_list)} # 헤더의 user-agent도 랜덤 선택 
