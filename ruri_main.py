...
collection_list = ['realcook', 'ilbe',
                   'realmlbpark', 'ygosu2', 'ruri', 'realclien']
target_list = ['cook', 'ilbe', 'MPark', 'ygosu2', 'ruriweb', 'clien']
...
for i in range(len(collection_list)):
    try:
        ...
        # (콜렉션명,목표, 마지막페이지, (옵션 : 원하는 페이지 분할 개수), (옵션 : 첫 번째 페이지))
        cr.crawling(collection, target, lastpage, dividePages, firstpage)

        # 프로그램 종료 측정 및 결과 출력
        print('It takes %s seconds completing the crawling and the uploading' %
