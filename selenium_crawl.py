...
class selenium_WebCrawler:
    ...
    # 상단 페이지의 정보 크롤링
    def cr_upperpages(self, url, headers, lastpage, keyvalues, start_time, pagetype, startini = 0, endini = 6): # 페이지 parar 이름이 다른 사이트가 있어서 일단 추가
    ...
    def cr_lowerpages(self, headers, upper_page_list, keykeys, keyvalues, startini=6, endini=12):
        ...
        if platform.system() != "Linux":
            ...
            wd = webdriver.Chrome(executable_path=r'C:\Program Files (x86)\Google\Chrome\Application\chromedriver.exe', options=options)
    ...
    def selenium_crawlingposts(self, lastpage, cvalues, pagetype): # 역시 여기서도 pagetype 설정 추가
        ...
        #################################
        # 1. upper page - 상단 페이지 실행
        upper_page_list = self.cr_upperpages(url, headers, lastpage, keyvalues, start_time, pagetype)
        u_time = time.time()
